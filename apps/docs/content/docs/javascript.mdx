...
strings. DOCX, XLSX, and PPTX also ship a `-node` package that calls the native
Rust facade. There is no Vue package — React, framework-free JavaScript, and
native Node.js are the supported entry points.
...
## Run natively in Node.js

```bash
npm install @betteroffice/xlsx-node
```

```ts
import { readFile } from "node:fs/promises";
import { openWorkbook } from "@betteroffice/xlsx-node";

const workbook = await openWorkbook(await readFile("report.xlsx"));
workbook.set(0, "B10", "=SUM(B1:B9)");
...
objects, and move opening, layout, rasterization, and saving off the JavaScript
event loop. Platform binaries are optional dependencies selected by npm, so
installing needs no Rust toolchain and downloads no executable at install time.
Use `@betteroffice/docx-node`, `@betteroffice/xlsx-node`, or
`@betteroffice/pptx-node` for the corresponding Rust facade.
...
| `@betteroffice/docx-node` | Native Node.js binding for DOCX inspection, editing, layout, PNG rendering, and saving. |
...
| `@betteroffice/xlsx-node` | Native Node.js binding for XLSX calculation, editing, collaboration, proposals, PNG rendering, and saving. |
...
| `@betteroffice/pptx-node` | Native Node.js binding for PPTX inspection, editing, collaboration, proposals, PNG rendering, and saving. |
