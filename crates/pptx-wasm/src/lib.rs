...
fn samples_to_rgba8(
    ...
    mut buffer: tiff::decoder::DecodingResult,
    ...
    match color {
        tiff::ColorType::Gray(8) if sample_bytes == 1 => {
            expect_samples(&chunky, pixels, 1, 1)?;
            Ok(gray_opaque(&chunky))
        }
        tiff::ColorType::Gray(bits) if sample_bytes == 1 && matches!(bits, 1 | 2 | 4) => {
            unpack_gray(&chunky, width, pixels, bits)
        }
        tiff::ColorType::Gray(16) if sample_bytes == 2 => {
            expect_samples(&chunky, pixels, 1, 2)?;
            Ok(gray16_opaque(&chunky))
        }
        tiff::ColorType::GrayA(8) if sample_bytes == 1 => {
            expect_samples(&chunky, pixels, 2, 1)?;
            Ok(gray_alpha(&chunky))
        }
        tiff::ColorType::GrayA(16) if sample_bytes == 2 => {
            expect_samples(&chunky, pixels, 2, 2)?;
            Ok(gray_alpha16(&chunky))
        }
        ...
        tiff::ColorType::RGB(8) if sample_bytes == 1 => {
            expect_samples(&chunky, pixels, 3, 1)?;
            Ok(rgb_opaque(&chunky))
        }
        tiff::ColorType::RGB(16) if sample_bytes == 2 => {
            expect_samples(&chunky, pixels, 3, 2)?;
            Ok(rgb16_opaque(&chunky))
        }
        tiff::ColorType::RGBA(8) if sample_bytes == 1 => {
            expect_samples(&chunky, pixels, 4, 1)?;
            Ok(chunky)
        }
        tiff::ColorType::RGBA(16) if sample_bytes == 2 => {
            expect_samples(&chunky, pixels, 4, 2)?;
            Ok(rgba16_to_rgba8(&chunky))
        }
        tiff::ColorType::CMYK(8) if sample_bytes == 1 => {
            expect_samples(&chunky, pixels, 4, 1)?;
            Ok(cmyk_opaque(&chunky))
        }
        tiff::ColorType::CMYK(16) if sample_bytes == 2 => {
            expect_samples(&chunky, pixels, 4, 2)?;
            Ok(cmyk16_opaque(&chunky))
        }
        tiff::ColorType::CMYKA(8) if sample_bytes == 1 => {
            expect_samples(&chunky, pixels, 5, 1)?;
            Ok(cmyka_opaque(&chunky))
        }
        tiff::ColorType::CMYKA(16) if sample_bytes == 2 => {
            expect_samples(&chunky, pixels, 5, 2)?;
            Ok(cmyka16_opaque(&chunky))
        }
        _ => Err("Unsupported TIFF color type".to_owned()),
    }
}
...
fn unsupported_compression(compression: u16) -> String {
    match tiff::tags::CompressionMethod::from_u16(compression) {
...
fn undo_predictor(raw: &mut [u8], row_bytes: usize, bits: u16, order: tiff::tags::ByteOrder) {
    if bits == 16 {
...
trait U16Bytes {
    fn from_bytes(pair: &[u8], order: tiff::tags::ByteOrder) -> u16;
    fn to_bytes(self, order: tiff::tags::ByteOrder) -> [u8; 2];
}

impl U16Bytes for u16 {
    fn from_bytes(pair: &[u8], order: tiff::tags::ByteOrder) -> u16 {
        match order {
            tiff::tags::ByteOrder::LittleEndian => u16::from_le_bytes([pair[0], pair[1]]),
            tiff::tags::ByteOrder::BigEndian => u16::from_be_bytes([pair[0], pair[1]]),
        }
    }

    fn to_bytes(self, order: tiff::tags::ByteOrder) -> [u8; 2] {
        match order {
            tiff::tags::ByteOrder::LittleEndian => self.to_le_bytes(),
            tiff::tags::ByteOrder::BigEndian => self.to_be_bytes(),
        }
    }
}
...
mod tests {
    ...
    where
        ...
        [Color::Inner]: tiff::encoder::TiffValue,
    {
    ...
    fn round_trip(tiff: &[u8]) -> (u32, u32, Vec<u8>) {
        decode_png(&decode_tiff(tiff).unwrap())
    }
    ...
    fn decodes_lzw_packbits_and_deflate() {
        ...
        for compression in [
            ...
            Compression::Deflate(tiff::encoder::DeflateLevel::Fast),
        ] {
    ...
    fn refuses_unsupported_compression() {
        for compression in [0xC350, 0xC351] {
            ...
            let error = decode_tiff(&tiff).unwrap_err();
